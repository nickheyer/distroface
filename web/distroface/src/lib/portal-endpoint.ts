// A portal endpoint is one value in three forms - host, host:port, or :port
export type ParsedEndpoint = { hostname: string; port: number; error: string };

const hostnamePattern = /^[a-z0-9]([a-z0-9.-]*[a-z0-9])?$/;

export function parseEndpoint(raw: string): ParsedEndpoint {
	const text = raw.trim().toLowerCase();
	if (text === '') return { hostname: '', port: 0, error: 'hostname or port required' };

	const idx = text.lastIndexOf(':');
	const hostname = idx === -1 ? text : text.slice(0, idx);
	let port = 0;
	if (idx !== -1) {
		const portPart = text.slice(idx + 1);
		if (!/^\d+$/.test(portPart)) return { hostname, port: 0, error: 'invalid port' };
		port = Number(portPart);
		if (port < 1 || port > 65535) return { hostname, port: 0, error: 'port must be between 1 and 65535' };
	}
	if (hostname !== '' && !hostnamePattern.test(hostname)) {
		return { hostname, port, error: 'invalid hostname' };
	}
	return { hostname, port, error: '' };
}

export function formatEndpoint(hostname: string, port: number): string {
	if (hostname === '') return `:${port}`;
	return port > 0 ? `${hostname}:${port}` : hostname;
}
