<script lang="ts">
  ...
  import { Package, Upload, Trash2, Plus, Search, Globe, Lock, Loader2 } from "lucide-svelte";
...
<div class="space-y-6">
  ...
  <div class="sm:flex sm:items-center sm:justify-between">
      <div>
          <h1 class="text-2xl font-semibold text-gray-900">Artifact Storage</h1>
  ...
  {:else}
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {#each artifacts.filtered as repo}
              <div class="relative rounded-lg border border-gray-300 bg-white px-6 py-5 shadow-sm hover:border-gray-400 focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-2">
                  <div class="flex items-center space-x-3">
                      <div class="flex-shrink-0">
                          <Package class="h-6 w-6 text-gray-400" />
                      </div>
                      <div class="min-w-0 flex-1">
                          <a href={`/artifacts/${repo.name}`} class="focus:outline-none">
                              <span class="absolute inset-0" aria-hidden="true"></span>
                              <p class="text-sm font-medium text-gray-900">{repo.name}</p>
                              <p class="truncate text-sm text-gray-500">{repo.description}</p>
                          </a>
                      </div>
                      <div class="flex-shrink-0">
                          {#if repo.private}
                          ...
                          {:else}
                              <Globe class="h-4 w-4 text-green-400" />
                          {/if}
                      </div>
                  </div>
