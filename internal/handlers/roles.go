...
func NewRoleHandler(repo repository.Repository, permManager *permissions.PermissionManager, log *logging.LogService) *RoleHandler {
	return &RoleHandler{
		repo:        repo,
...
func (h *RoleHandler) ListRoles(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.Printf("Failed to list roles: %v", err)
	...
	if err := json.NewEncoder(w).Encode(roles); err != nil {
		h.log.Printf("Failed to encode roles response: %v", err)
...
func (h *RoleHandler) CreateRole(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&roleRequest); err != nil {
		h.log.Printf("Failed to decode role request: %v", err)
	...
	if err := h.repo.CreateRole(&roleRequest); err != nil {
		h.log.Printf("Failed to create role: %v", err)
...
func (h *RoleHandler) DeleteRole(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.repo.GetRole(name); err != nil {
		h.log.Printf("Role not found: %v", err)
		http.Error(w, "ROLE NOT FOUND", http.StatusNotFound)
		return
	}

	if err := h.repo.DeleteRole(name); err != nil {
		h.log.Printf("Failed to delete role: %v", err)
...
func (h *RoleHandler) UpdateRole(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&roleRequest); err != nil {
		h.log.Printf("Failed to decode role update request: %v", err)
		http.Error(w, "INVALID REQUEST", http.StatusBadRequest)
		return
	}
	...
	for _, perm := range roleRequest.Permissions {
		if !isValidAction(perm.Action) || !isValidResource(perm.Resource) {
			h.log.Printf("Invalid permission: Action=%s, Resource=%s", perm.Action, perm.Resource)
			http.Error(w, "INVALID PERMISSION", http.StatusBadRequest)
			return
		}

		if perm.Scope != "" && !isValidScope(perm.Scope) {
			h.log.Printf("Invalid scope: %s", perm.Scope)
	...
	if err := h.repo.UpdateRole(updatedRole); err != nil {
		h.log.Printf("Failed to update role: %v", err)
